...
import org.junit.Test // @GOOSE-01
...
class ExtractionsTest {
    ...
    @Test
    def cnn1( ) {
        implicit val context = TestUtils.DEFAULT_CONFIG
    ...
    def testHuffingtonPost( ) {
        ...
        val description = "A top regional Federal Reserve official sharply criticized Friday the Fed's ongoing policy of keeping interest rates near zero -- and at record lows " +
    ...
    */
    ...
    def issue28( ) {
        ...
        val article = TestUtils.getArticle( url, html )
        TestUtils.runArticleAssertions( article = article, expectedStart = "Emergency services were called to Kismot Restaurant's curry-eating challenge," )
    }
    ...
    def issue32( ) {
        ...
        val article = TestUtils.getArticle( url, html )
        TestUtils.runArticleAssertions( article = article, expectedStart = "Opposition to a proposal to remove certain personal data" )
    }
}